...
class StandardRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 ral_path=None,
                 random_q=None,
                 triplet_lambda_sim=None, 
                 triplet_lambda_dis=None, 
                 triplet_margin_sim=None, 
                 triplet_margin_dis=None, 
                 triplet_beta_sim=None, 
                 triplet_beta_dis=None,
                 use_raf=False, 
                 raf_topk=20,
                 raf_box_level=False,
                 raf_weight='',
        ...
        # if False:
            if not self.fix_bg:
                self.text_features_for_classes = torch.load(save_path, map_location=device).to().squeeze()[:self.num_classes] # to avoid loading on duplicate cuda gpu. 
            else:
        ...
        if self.use_ral:
            ral_info = torch.load(ral_path, map_location=device)
            ...
            self.hard_pad = torch.tensor(ral_info['pad_out_sim'])
            self.easy_pad = torch.tensor(ral_info['pad_out_dis'])
            del ral_info
            
            self.anchor_weight = F.normalize(anchor_weight, p=2, dim=1)
            self.hard_weight = F.normalize(hard_weight, p=2, dim=2)
            self.easy_weight = F.normalize(easy_weight, p=2, dim=2)

            self.random_q = random_q
            self.triplet_lambda_sim = triplet_lambda_sim
            self.triplet_lambda_dis = triplet_lambda_dis
            self.triplet_margin_sim = triplet_margin_sim
            self.triplet_margin_dis = triplet_margin_dis
            self.triplet_beta_sim = triplet_beta_sim
            self.triplet_beta_dis = triplet_beta_dis
            
            print('-------- RAL triplet loss --------')
            print('ral path', ral_path)
            print('random q', random_q)
            print('triplet sim strength', triplet_lambda_sim)
            print('triplet dis strength', triplet_lambda_dis)
            print('triplet sim margin', triplet_margin_sim)
            print('triplet dis margin', triplet_margin_dis)
            print('triplet beta sim', triplet_beta_sim)
            print('triplet beta dis', triplet_beta_dis)
            print('-------------------------------------')

        self.use_raf = use_raf
        if use_raf:
            cbm_model = CBM()
            cbm_model.load_state_dict(torch.load(raf_weight, map_location='cpu'))
            cbm_model.eval()
            self.cbm_model = cbm_model

            concept_pkl = torch.load('ralf/v3det_gpt_noun_chunk_lvis_strict.pkl', map_location='cpu')
            ...
            print('----------- RAF configs -----------')
            print('raf topk', raf_topk)
            print('raf box level', raf_box_level)
            print('raf weight', raf_weight)
            print('-------------------------------------')
    ...
    def _bbox_forward_train(self, x, img, img_no_normalize, sampling_results, proposals_pre_computed, gt_bboxes, gt_labels,
        ...
        if self.use_ral:
            gt_boxes_roi = bbox2roi(gt_bboxes)
            concat_gt_labels = torch.cat(gt_labels)
            batch = gt_boxes_roi.shape[0]
            random_q = self.random_q
            device_roi = gt_boxes_roi.device

            assert torch.cat(gt_bboxes).shape[0] > 0, 'no gt bboxes!!'
            assert torch.sum(concat_gt_labels < 0) == 0
            assert torch.sum(concat_gt_labels > 1202) == 0

            _, gt_region_embeddings = self._bbox_forward(x, gt_boxes_roi)
            gt_region_embeddings = self.projection(gt_region_embeddings)
            gt_region_embeddings = torch.nn.functional.normalize(gt_region_embeddings, p=2, dim=1)
            
            dimdim = self.hard_weight.shape[2]
            len_hard = self.hard_pad[concat_gt_labels]
            len_easy = self.easy_pad[concat_gt_labels]
            random_hard = []
            random_easy = []
            for limit in len_hard:
                random_hard.append(torch.randperm(limit, device=device_roi)[:random_q])
            for limit in len_easy:
                random_easy.append(torch.randperm(limit, device=device_roi)[:random_q])
            random_hard = torch.stack(random_hard)
            random_easy = torch.stack(random_easy)
            batch_hard_weight_ = self.hard_weight[concat_gt_labels].to(device=device_roi)
            random_hard_ = random_hard.unsqueeze(-1).expand(-1, -1, dimdim)
            batch_hard_weight = batch_hard_weight_.gather(1, random_hard_)
            batch_easy_weight_ = self.easy_weight[concat_gt_labels].to(device=device_roi)
            random_easy_ = random_easy.unsqueeze(-1).expand(-1, -1, dimdim)
            batch_easy_weight = batch_easy_weight_.gather(1, random_easy_)
            
            batch_anchor_weight = self.anchor_weight[concat_gt_labels]

            s_anchor = gt_region_embeddings @ batch_anchor_weight.to(device=device_roi).T
            s_anchor = s_anchor.gather(1, torch.arange(batch, device=device_roi).view(-1, 1))
            s_hard_neg = gt_region_embeddings @ batch_hard_weight.view(batch * random_q, -1).to(device=device_roi).T
            s_hard_neg = s_hard_neg.gather(1, torch.arange(s_hard_neg.shape[1], device=device_roi).view(-1, random_q))

            s_easy_neg = gt_region_embeddings @ batch_easy_weight.view(batch * random_q, -1).to(device=device_roi).T
            s_easy_neg = s_easy_neg.gather(1, torch.arange(s_easy_neg.shape[1], device=device_roi).view(-1, random_q))

            anchor = s_anchor.squeeze(1)
            hard_neg = s_hard_neg.mean(dim=1)
            easy_neg = s_easy_neg.mean(dim=1) * self.triplet_lambda_dis

            loss_hard_neg = torch.clamp(self.triplet_margin_sim + hard_neg  * self.triplet_lambda_sim - anchor, 0)
            loss_easy_neg = torch.clamp(self.triplet_margin_dis + easy_neg - hard_neg, 0)

            loss_bbox.update(hard_neg_loss=loss_hard_neg.mean() * self.triplet_beta_sim, easy_neg_loss=loss_easy_neg.mean() * self.triplet_beta_dis)
        ...
        if self.use_raf:
            if self.raf_box_level:
                ...
                with torch.no_grad():
                    ...
                    sim_gen_gt = (gen @ text_features.T).squeeze(0)
                    sim_v_t = sim_gen_gt
                k = self.raf_topk
                ...
                zs_scores = sim_v_t
            cls_score = cls_score * (zs_scores.sigmoid() - 0.25)
