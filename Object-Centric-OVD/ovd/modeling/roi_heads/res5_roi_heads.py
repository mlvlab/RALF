...
class CustomRes5ROIHeads(Res5ROIHeads):
    ...
    def forward(self, images, features, proposals, targets=None, ann_type='box'):
        ...
        if self.training and self.use_ral and ann_type == 'box':
            ...
            assert torch.sum(gt_classes > 79) == 0
            assert gt_classes.shape[0] > 0
                
            hnen_box_features = self._shared_roi_transform([features[f] for f in self.in_features], boxes_for_hnen)
            hnen_box_embeds = hnen_box_features.mean(dim=[2, 3])
