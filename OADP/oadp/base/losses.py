...
class HenTripletLoss(todd.losses.BaseLoss): # NOTE: RAL
    def __init__(
        self,
        *args,
        margin: float = 0.0,
        lambda_stren: float = 1.0,
        beta: float = 1.0,
        **kwargs,
    ):
        super().__init__(*args, **kwargs)
        self._margin = margin
        self._lambda = lambda_stren
        self._beta = beta
    def forward(
        self,
        s_pos: torch.Tensor,
        ...
        **kwargs,
        ...
        if with_anchor:
            ...
            pos = s_pos.mean(dim=1)
        neg = s_neg.mean(dim=1)

        loss_ = torch.clamp(self._margin + neg * self._lambda - pos, 0)
        return loss_.mean() * self._beta
